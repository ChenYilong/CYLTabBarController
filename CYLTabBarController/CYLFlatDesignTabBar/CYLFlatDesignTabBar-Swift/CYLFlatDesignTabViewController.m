//
//  CYLFlatDesignTabViewController.m
//  CYLFlatDesignTabViewController
//
...
CGFloat const CYLFlatDesignTabViewControllerHideShowBarDuration = 0.2;

@interface CYLFlatDesignTabViewController ()<UINavigationControllerExtensionDelegate>
...
@implementation CYLFlatDesignTabViewController {
    struct {
...
#pragma mark - Setter
- (void)setDelegate:(id<CYLFlatDesignTabViewControllerDelegate>)delegate {
    _delegate = delegate;
...
- (void)setViewControllers:(NSArray<__kindof UIViewController *> *)viewControllers {
    ...
    if (self.selectedIndex == NSNotFound) {
        ...
        if (!_selectedViewController) {
            ...
            _cyl_tabBar.selectedItem = _selectedViewController.cyl_tabBarItem;
        }
    }
}
...
- (void)setSelectedViewController:(__kindof UIViewController *)selectedViewController {
    ...
    if ([self.viewControllers containsObject:selectedViewController]) {
        ...
        NSLog(@"-[CYLFlatDesignTabViewController setSelectedViewController:] only a view controller in the tab bar controller's list of view controllers can be selected.");
    }
}
...
- (void)setTabBarHidden:(BOOL)hidden animated:(BOOL)animated {
    ...
    if (_tabBarHidden != hidden) {
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:CYLFlatDesignTabViewControllerHideShowBarDuration animations:^{
...
- (BOOL)_checkHidesBottomBarWhenPushed {
    ...
    if ([selectedViewController isKindOfClass:[UINavigationController class]]) {
        ...
        NSInteger currentIndex =
            [navigationController.viewControllers indexOfObject:navigationController.topViewController];
        
        NSInteger showsBottomBarIndex = -1;
        
        for (NSInteger index = 0;
             index < navigationController.viewControllers.count;
             index++) {
            
            UIViewController *viewController =
                navigationController.viewControllers[index];
...
- (void)_tabBarItemClicked:(CYLFlatDesignTabBarItem *)tabBarItem {
...
- (void)_cycleFromSourceViewController:(nullable UIViewController *)sourceViewController
    ...
    if (!animator) {
        animator = [[_CYLFlatDesignTabBarControllerTransitionAnimator alloc] init];
    }
    
    _CYLFlatDesignTabBarControllerTransitionContext *transitionContext = [[_CYLFlatDesignTabBarControllerTransitionContext alloc] initWithSourceViewController:sourceViewController destinationViewController:destinationViewController containerView:self.containerView];
...
@implementation UIViewController (CYLFlatDesignTabViewControllerItem)
...
- (void)cyl_setTabBarItem:(CYLFlatDesignTabBarItem *)tabBarItem {
    ...
    [self.cyl_tabBarController _changeItem:oldItem toItem:tabBarItem];

    objc_setAssociatedObject(self, &_qqtabBarItemPropertyKey, tabBarItem, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CYLFlatDesignTabViewController *)cyl_tabBarController {
    return [self cyl_nearestParentViewControllerThatIsKindOf:[CYLFlatDesignTabViewController class]];
}

- (CYLFlatDesignTabViewController *)cyl_nearestParentViewControllerThatIsKindOf:(Class)c {
    UIViewController *controller = self.parentViewController;
    while (controller && ![controller isKindOfClass:c]) {
        controller = controller.parentViewController;
    }
    if (controller && [controller isKindOfClass:c]) {
        return (CYLFlatDesignTabViewController *)controller;
    }
    return nil;
}
