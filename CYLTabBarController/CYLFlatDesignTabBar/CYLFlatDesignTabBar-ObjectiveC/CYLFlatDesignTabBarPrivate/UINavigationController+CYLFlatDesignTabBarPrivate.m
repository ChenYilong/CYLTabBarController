...
// 是否嵌套在CYLTabBarController中

@property (nonatomic, assign, getter=cylflatdesign_nestedInCYLFlatDesignUITabBarController, setter=cylflatdesign_setNestedInCYLFlatDesignUITabBarController:) BOOL cylflatdesign_nestedInCYLFlatDesignUITabBarController;
...
- (BOOL)cylflatdesign_nestedInCYLFlatDesignUITabBarController {
    NSNumber *nestedInCYLFlatDesignUITabBarControllerObject = objc_getAssociatedObject(self, @selector(cylflatdesign_nestedInCYLFlatDesignUITabBarController));
    return [nestedInCYLFlatDesignUITabBarControllerObject boolValue];
}

- (void)cylflatdesign_setNestedInCYLFlatDesignUITabBarController:(BOOL)nestedInCYLFlatDesignUITabBarController {
    NSNumber *nestedInCYLFlatDesignUITabBarControllerObject = @(nestedInCYLFlatDesignUITabBarController);
    objc_setAssociatedObject(self, @selector(cylflatdesign_nestedInCYLFlatDesignUITabBarController), nestedInCYLFlatDesignUITabBarControllerObject, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
#pragma mark - 主流程
/*!
 *该方法会在 CYLFlatDesignTabBar 手动调用， 减少侵入性。否则可能导致液态样式也被影响。
 * //+ (void)load {
...
- (UIViewController *)cylflatdesign_popViewControllerAnimated:(BOOL)animated {
    ...
    if (!self.cylflatdesign_nestedInCYLFlatDesignUITabBarController) {
...
- (void)cylflatdesign_didMoveToParentViewController:(nullable UIViewController *)parent {
    ...
    if (parent == nil && self.cylflatdesign_nestedInCYLFlatDesignUITabBarController) {
        [self cylflatdesign_setExtensionDelegate:nil];
        self.cylflatdesign_nestedInCYLFlatDesignUITabBarController = NO;
        [self _cylflatdesign_unregisterPopGestureRecognizer];
    }
    else if ([parent cyl_isFlatDesignStyleTabBar]) {
        [self cylflatdesign_setExtensionDelegate:(id<UINavigationControllerExtensionDelegate>)parent];
        
        self.cylflatdesign_nestedInCYLFlatDesignUITabBarController = YES;
        [self cylflatdesign_registerPopGestureRecognizer];
        [self cylflatdesign_updateNavigationBarHeight];
    }
    /*!
     * else if ([parent cyl_isFlatDesignStyleTabBar] || [parent isKindOfClass:[CYLFlatDesignUIViewController class]]) {
...
- (void)_cylflatdesign_popGestureRecognizerHandler:(UIPanGestureRecognizer *)popGestureRecognizer {
    SEL action = @selector(cylflatdesign_navigationController:didUpdateInteractiveFrom:to:popGestureRecognizer:);
    if ([self.cylflatdesign_extensionDelegate respondsToSelector:action]) {
    ...
    action = @selector(cylflatdesign_navigationController:didUpdateInteractiveFrom:to:percentComplete:);
    if ([self.cylflatdesign_extensionDelegate respondsToSelector:action]) {
        [self.cylflatdesign_extensionDelegate cylflatdesign_navigationController:self
                                                        didUpdateInteractiveFrom:[self.transitionCoordinator viewControllerForKey:UITransitionContextFromViewControllerKey]
                                                                              to:self.topViewController
...
- (void)_cylflatdesign_pushViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC animated:(BOOL)animated {
    ...
    SEL action = @selector(cylflatdesign_navigationController:didBeginTransitionFrom:to:operation:);
    if ([self.cylflatdesign_extensionDelegate respondsToSelector:action]) {
        [self.cylflatdesign_extensionDelegate cylflatdesign_navigationController:self
    ...
    if (transitionCoordinator) {
        ...
        [transitionCoordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
            ...
            SEL action = @selector(cylflatdesign_navigationController:willEndTransitionFrom:to:operation:cancelled:);
            if ([strongSelf.cylflatdesign_extensionDelegate respondsToSelector:action]) {
            ...
            if (weakSelf == nil) { return; }
            typeof(self) strongSelf = weakSelf;
            SEL action = @selector(cylflatdesign_navigationController:didEndTransitionFrom:to:operation:cancelled:);
            if ([strongSelf.cylflatdesign_extensionDelegate respondsToSelector:action]) {
                [strongSelf.cylflatdesign_extensionDelegate cylflatdesign_navigationController:strongSelf
        ...
        if (animated) {
            ...
            action = @selector(cylflatdesign_navigationController:didEndTransitionFrom:to:operation:cancelled:);
            if ([self.cylflatdesign_extensionDelegate respondsToSelector:action]) {
                [self.cylflatdesign_extensionDelegate cylflatdesign_navigationController:self
                                                                    didEndTransitionFrom:fromVC
                                                                                      to:toVC
...
- (void)cylflatdesign_updateNavigationBarHeight {
    ...
    SEL action = @selector(cylflatdesign_navigationController:navigationBarDidChangeHeight:);
    if ([self.cylflatdesign_extensionDelegate respondsToSelector:action]) {
